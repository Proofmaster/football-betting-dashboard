function showDetailModal(match) {
  ...
  const h2hHtml = match.details && match.details.h2h && match.details.h2h.length > 0 
    ? match.details.h2h.map(m => `
        <div class="bg-gray-800 p-4 rounded-2xl text-center">
          ...
          <div class="font-medium">${m.home} ${m.score} ${m.away}</div>
        </div>`).join('')
    : '<div class="bg-gray-800 p-8 rounded-2xl text-center text-gray-400">H2H data loading from multiple seasons...</div>';

  const homeFormHtml = match.details && match.details.home_form && match.details.home_form.length > 0 
    ? match.details.home_form.map(f => `
        <div class="bg-gray-800 p-4 rounded-2xl flex justify-between items-center">
          <span>${f.opponent}</span>
          <span class="font-mono">${f.score}</span>
        </div>`).join('')
    : '<div class="text-gray-400 p-6">Recent home form loading...</div>';
  ...
  const awayFormHtml = match.details && match.details.away_form && match.details.away_form.length > 0 
    ...
    : '<div class="text-gray-400 p-6">Recent away form loading...</div>';
  ...
  const html = `
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div>
        ...
        <div class="bg-gray-800 rounded-2xl p-6 space-y-4">
          ...
          <div class="pt-4 border-t border-gray-700 grid grid-cols-3 text-center text-sm">
    ...
    <div class="mt-12">
      <h3 class="font-semibold mb-4">Head-to-Head (last meetings)</h3>
